...
export default async function EngineeringPage({ searchParams }: PageProps) {
  ...
  const faq = [
    {
      q: t(locale, { pt: "O que é paginação cursor-based e quando usar?", en: "What is cursor-based pagination and when should you use it?", es: "¿Qué es la paginación cursor-based y cuándo usarla?", jp: "カーソルベースのページネーションとは何ですか？いつ使うべきですか？", fr: "Qu'est-ce que la pagination cursor-based et quand l'utiliser ?" }),
      a: t(locale, { pt: "Em vez de OFFSET/LIMIT, cada página é buscada a partir de um cursor — um ponteiro estável para o último registro carregado (ex.: created_at + id). Isso mantém o custo de leitura constante em listas grandes e evita itens duplicados ou pulados quando novos registros chegam durante a navegação. É a escolha certa para feeds e inboxes; o trade-off é perder o salto direto para uma página arbitrária.", en: "Instead of OFFSET/LIMIT, each page is fetched from a cursor — a stable pointer to the last loaded record (e.g., created_at + id). This keeps read cost constant on large lists and prevents duplicated or skipped items when new records arrive mid-navigation. It's the right choice for feeds and inboxes; the trade-off is losing direct jumps to an arbitrary page.", es: "En lugar de OFFSET/LIMIT, cada página se busca a partir de un cursor — un puntero estable al último registro cargado (ej.: created_at + id). Esto mantiene el costo de lectura constante en listas grandes y evita ítems duplicados o saltados cuando llegan nuevos registros durante la navegación. Es la elección correcta para feeds e inboxes; el trade-off es perder el salto directo a una página arbitraria.", jp: "OFFSET/LIMITの代わりに、各ページはカーソル — 最後に読み込んだレコードへの安定したポインタ（例：created_at + id）— から取得します。大きなリストでも読み取りコストが一定に保たれ、ナビゲーション中に新しいレコードが追加されても項目の重複や抜けを防げます。フィードやインボックスに最適な選択です。トレードオフは任意のページへ直接ジャンプできなくなることです。", fr: "Au lieu d'OFFSET/LIMIT, chaque page est récupérée à partir d'un cursor — un pointeur stable vers le dernier enregistrement chargé (ex. : created_at + id). Cela maintient le coût de lecture constant sur les grandes listes et évite les éléments dupliqués ou sautés quand de nouveaux enregistrements arrivent pendant la navigation. C'est le bon choix pour les feeds et les inboxes ; le trade-off est de perdre le saut direct vers une page arbitraire." }),
    },
    {
      q: t(locale, { pt: "O que é arquitetura multi-tenant?", en: "What is multi-tenant architecture?", es: "¿Qué es la arquitectura multi-tenant?", jp: "マルチテナントアーキテクチャとは何ですか？", fr: "Qu'est-ce que l'architecture multi-tenant ?" }),
      a: t(locale, { pt: "Um padrão de design onde múltiplas organizações compartilham a mesma aplicação e banco de dados, mas os dados de cada tenant são isolados. A abordagem mais comum em SaaS moderno é banco compartilhado com coluna tenant_id e Row Level Security (RLS) do PostgreSQL como rede de segurança.", en: "A design pattern where multiple organizations share the same application and database, but each tenant's data is isolated. The most common approach in modern SaaS is shared database with tenant_id column and PostgreSQL Row Level Security (RLS) as a safety net.", es: "Un patrón de diseño donde múltiples organizaciones comparten la misma aplicación y base de datos, pero los datos de cada tenant están aislados. El enfoque más común en SaaS moderno es base de datos compartida con columna tenant_id y Row Level Security (RLS) de PostgreSQL como red de seguridad.", jp: "複数の組織が同じアプリケーションとデータベースを共有しながら、各テナントのデータは分離される設計パターンです。最近のSaaSで最も一般的なアプローチは、tenant_id カラムを持つ共有データベースと、セーフティネットとしての PostgreSQL の Row Level Security（RLS）です。", fr: "Un design pattern où plusieurs organisations partagent la même application et la même base de données, mais où les données de chaque tenant sont isolées. L'approche la plus courante dans le SaaS moderne est une base partagée avec une colonne tenant_id et le Row Level Security (RLS) de PostgreSQL comme filet de sécurité." }),
    },
    {
      q: t(locale, { pt: "Como lidar com webhooks de pagamento de forma confiável?", en: "How to handle payment webhooks reliably?", es: "¿Cómo manejar webhooks de pago de forma confiable?", jp: "決済Webhookを信頼性高く処理するには？", fr: "Comment traiter les webhooks de paiement de façon fiable ?" }),
      a: t(locale, { pt: "Use uma abordagem em camadas: valide assinaturas em cada evento, garanta idempotência com IDs de eventos armazenados, responda imediatamente e processe em background, valide transições de estado com state machine, execute jobs de reconciliação periódicos e direcione eventos com falha para uma dead letter queue.", en: "Use a layered approach: validate signatures on every event, enforce idempotency with stored event IDs, ack immediately and process in background, validate state transitions with a state machine, run periodic reconciliation jobs, and route failed events to a dead letter queue.", es: "Use un enfoque en capas: valide firmas en cada evento, garantice idempotencia con IDs de eventos almacenados, responda inmediatamente y procese en background, valide transiciones de estado con state machine, ejecute jobs de reconciliación periódicos y dirija eventos fallidos a una dead letter queue.", jp: "多層的なアプローチを使います：すべてのイベントで署名を検証し、保存したイベントIDで冪等性を担保し、即座にackを返してバックグラウンドで処理し、ステートマシンで状態遷移を検証し、定期的な照合ジョブを実行し、失敗したイベントはデッドレターキューへ振り分けます。", fr: "Utilisez une approche en couches : validez les signatures sur chaque événement, garantissez l'idempotence avec des IDs d'événements stockés, répondez immédiatement et traitez en background, validez les transitions d'état avec une state machine, exécutez des jobs de réconciliation périodiques et dirigez les événements en échec vers une dead letter queue." }),
    },
    {
      q: t(locale, { pt: "Como integrar IA em sistemas de produção?", en: "How to integrate AI into production systems?", es: "¿Cómo integrar IA en sistemas de producción?", jp: "本番システムにAIを統合するには？", fr: "Comment intégrer l'IA dans des systèmes de production ?" }),
      a: t(locale, { pt: "Trate a IA como um componente do sistema, não uma feature isolada. Processe mensagens de forma assíncrona, registre dados de volta no banco, implemente fallbacks para quando o LLM estiver indisponível e monitore a qualidade das respostas. O ponto central é confiabilidade — o sistema precisa funcionar mesmo quando o provedor de IA tem problemas.", en: "Treat AI as a system component, not a standalone feature. Process messages asynchronously, register data back into your database, implement fallbacks for when the LLM is unavailable, and monitor response quality. The key is reliability — the system must work even when the AI provider has issues.", es: "Trate la IA como un componente del sistema, no una feature aislada. Procese mensajes de forma asíncrona, registre datos de vuelta en la base de datos, implemente fallbacks para cuando el LLM no esté disponible y monitoree la calidad de las respuestas. El punto central es confiabilidad — el sistema debe funcionar incluso cuando el proveedor de IA tiene problemas.", jp: "AIを単独の機能ではなく、システムの一部として扱います。メッセージを非同期で処理し、データをデータベースへ書き戻し、LLMが利用できないときのフォールバックを実装し、応答品質を監視します。要点は信頼性です — AIプロバイダーに問題があってもシステムは動作し続けなければなりません。", fr: "Considérez l'IA comme un composant du système, pas comme une feature isolée. Traitez les messages de façon asynchrone, réécrivez les données dans la base, implémentez des fallbacks au cas où le LLM serait indisponible et surveillez la qualité des réponses. Le point central est la fiabilité — le système doit fonctionner même quand le fournisseur d'IA a des problèmes." }),
    },
    {
      q: t(locale, { pt: "O que é o padrão circuit breaker?", en: "What is the circuit breaker pattern?", es: "¿Qué es el patrón circuit breaker?", jp: "サーキットブレーカーパターンとは何ですか？", fr: "Qu'est-ce que le pattern circuit breaker ?" }),
      a: t(locale, { pt: "Um padrão de resiliência para integrações com APIs terceiras. Quando uma API externa começa a falhar, o circuit breaker 'abre' e retorna respostas fallback ao invés de propagar a falha pelo sistema. Após um período de cooldown, permite requisições de teste para verificar se o serviço se recuperou.", en: "A resilience pattern for third-party API integrations. When an external API starts failing, the circuit breaker 'opens' and returns fallback responses instead of cascading the failure through your system. After a cooldown period, it allows test requests to check if the service recovered.", es: "Un patrón de resiliencia para integraciones con APIs de terceros. Cuando una API externa empieza a fallar, el circuit breaker se 'abre' y retorna respuestas fallback en lugar de propagar la falla por el sistema. Después de un período de cooldown, permite solicitudes de prueba para verificar si el servicio se recuperó.", jp: "サードパーティAPI連携のためのレジリエンスパターンです。外部APIが失敗し始めると、サーキットブレーカーが「開き」、障害をシステム全体に波及させる代わりにフォールバック応答を返します。クールダウン期間の後、サービスが回復したかを確認するためのテストリクエストを許可します。", fr: "Un pattern de résilience pour les intégrations avec des APIs tierces. Quand une API externe commence à échouer, le circuit breaker « s'ouvre » et renvoie des réponses fallback au lieu de propager la panne à travers le système. Après une période de cooldown, il autorise des requêtes de test pour vérifier si le service est rétabli." }),
    },
  ]
